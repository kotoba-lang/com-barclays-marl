(ns barclays-marl.main
  "Kotodama WASM entrypoint for the Barclays_marl clean-room actor (L4) — Clojure port.
  ...
  facts (`barclays_marl.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "barclays_marl")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "barclays_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}                    :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "barclays_loa"
...
(defn emit-facts
  "EAVT facts for one record: {\"barclays_marl.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "barclays_marl-compat" :tier tier :entities entities} 200])
...
(defn start! [] :barclays_marl-compat/ready)
